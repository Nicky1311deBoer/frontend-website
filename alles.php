...
<body>
    ...
    <div class="kopje">
        ...
        <div class="kopjeselect">
            <h1>Soorteren</h1>
            <form action="alles.php" method="post" class="categorybuton">
                <button name="sieraden" class="button">Sieraden</button>
                <button name="kleding" class="button">Kleding</button>
                <button name="accesoires" class="button">Accesoires</button>
                <button name="alles" class="button">Alles</button>
            </form>
        </div>
    </div>
        ...
        <div class="alleproducten">
            <?php
                if(isset($_POST["sieraden"])) {
                    ...
                    $sql1 = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE size = 'S'");
                    $sql1->execute();
                    foreach ($sql1 as $row) {
                        echo "<div class='product'><a class='linktoproduct' href='product.php?id=" . $row['id'] . "'><img class='fotoproduct' src= fotos/" . $row['foto'] . "><p>" . $row['naam'] . "</p><p>" . $row['prijs'] . "</p></a></div>";
                    }
                    $sql2 = $pdo->query("SELECT foto, naam, prijs, id FROM accesoires");
                    $sql2->execute();
                    foreach ($sql2 as $row) {
                        echo "<div class='product'><a class='linktoproduct' href='product.php?id=" . $row['id'] . "'><img class='fotoproduct' src= fotos/" . $row['foto'] . "><p>" . $row['naam'] . "</p><p>" . $row['prijs'] . "</p></a></div>";
                    }
