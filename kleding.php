...
<body>
    ...
    <div class="kopje">
        ...
        <div class="kopjeselect">
            <h1>Soorteren</h1>
            <form action="kleding.php" method="post" class="categorybuton">
                <button name="shirt" class="button">Shirt's</button>
                <button name="rok" class="button">Rokken</button>
                <button name="trui" class="button">Truien</button>
                <button name="alles" class="button">Alle kleding</button>
            </form>
        </div>
    </div>
        ...
        <div class="alleproducten">
            <?php
                if(isset($_POST["shirt"])) {
                    $sql = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE catergory = 'shirt' AND size = 'S'");
                } elseif(isset($_POST["rok"])) {
                    $sql = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE catergory = 'rok' AND size = 'S'");
                } elseif(isset($_POST["trui"])) {
                    $sql = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE catergory = 'trui' AND size = 'S'");
                } elseif(isset($_POST["alles"])) {
                    $sql = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE size = 'S'");
                } else {
                    $sql = $pdo->query("SELECT foto, naam, prijs, size, id FROM kleding WHERE size = 'S'");
                }
                $sql->execute();
